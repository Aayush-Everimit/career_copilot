...
export function generateLocalAnalysis(
    ...
    const matchScore =
        jdKeywords.size === 0
            ...
            : Math.min(
                100,
                Math.round(
                    (matchedSkills.length / jdKeywords.size) * 100
                )
            );
    ...
    const resumeSummary =
        matchedSkills.length > 0
            ? `Your profile demonstrates experience related to ${matchedSkills
                .slice(0, 5)
                .join(", ")}. The strongest alignment with this role comes from these areas.`
    ...
    if (missingSkills.length > 0) {
        resumeSuggestions.push(
            `Consider highlighting experience related to ${missingSkills
                .slice(0, 3)
                .join(", ")}.`
        );
    }
    ...
    resumeSuggestions.push(
        "Quantify achievements with measurable results wherever possible."
    );

    resumeSuggestions.push(
        "Place the most relevant projects, experience, or accomplishments near the top."
    );

    const interviewQuestions =
        matchedSkills.length > 0
            ? matchedSkills.slice(0, 8).map(
                (skill) =>
                    `Describe your practical experience with ${skill} and how you have applied it in a real-world situation.`
            )
            : [
